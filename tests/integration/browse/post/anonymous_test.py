...
def test_post_anon_hidden_users(simple_users: ISimpleUsers):
    ...
    # Hidden in post list
    assert post.list(simple_users["user"]["token"])["posts"] == expect.Equals([
    ...
    # Hidden in post view
    p = post.view(simple_users["user"]["token"], post_id)
    assert p == expect.DictContainingItems({
        "post_id": post_id,
        "anonymous": True,
        "author": None,
    })
...
def test_post_anon_shown_mod(simple_users: ISimpleUsers):
    ...
    # Shown in post list
    assert post.list(simple_users["mod"]["token"])["posts"] == expect.Equals([
        expect.DictContainingItems({
            "post_id": post_id,
            "anonymous": True,
    ...
    # Shown in post view
    p = post.view(simple_users["mod"]["token"], post_id)
    assert p == expect.DictContainingItems({
        "post_id": post_id,
        "anonymous": True,
        "author": simple_users["user"]["user_id"],
